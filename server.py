...
# Helper function to get the effective API key
def get_api_keys(request, settings):
    """Get API keus from request or environment"""
...
async def check_plagiarism(request: PlagiarismRequest, settings: Settings = Depends(get_settings)):
    try:
        ...
        raise HTTPException(status_code=500, detail="fuzzywuzzy not installed. Instal with 'pip install fuzzywuzzy'")
    except HTTPException:
        raise 
    except Exception as e:
...
async def call_openai_api(prompt:str, api_key: str, model: str = "gpt-3.5-turbo") -> str:
    ...
    try:
        # Create a client with the specific API key
...
async def grade_text(request: GradeRequest, settings: Settings = Depends(get_settings)):
    try:
        ...
        grade = await call_openai_api(prompt, keys["openai-api-key"], model)
    except HTTPException:
        raise
    except Exception as e:
...
async def generate_feedback(request: GradeRequest, settings: Settings = Depends(get_settings)):
    try:
        ...
        feedback = await call_openai_api(prompt, keys["openai-api-key"], model)
        return feedback
    except HTTPException:
        raise 
    except Exception as e:
...
if __name__ == "__main__":
    ...
    logger.info("    - /tool/parse_file")
    logger.info("    - /tool/check_plagiarism")
    logger.info("    - /tool/grade_text")
    logger.info("    - /tool/generate_feedback")
